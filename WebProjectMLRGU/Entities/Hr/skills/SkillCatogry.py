...
class SkillCatogry(db.Entity):
    _table_ = ("bank_admin", "skill_catogry_lookup")
    ...
    def status(self , status):
        select(p.key for p in SkillCatogry if  self.status == status)

    @property
    @db_session(serializable=True)
    def findByKeyName(self , key):
        return select(p.key for p in SkillCatogry if self.key == key)

with db_session:
  # BankInfo who have a Id
  select(p.key for p in SkillCatogry if  p.findByKeyName(value))

  # persons who have a car
  select(p.status for p in SkillCatogry if  p.status(True))
