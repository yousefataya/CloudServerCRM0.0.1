...
from . import AppoiontmentInfo
db = Database()
db.bind(provider='oracle', user='bank_admin', password='opc@2020', dsn='ORCL')
db.generate_mapping(create_tables=True)
class AppointmentCatogry(db.Entity):
    _table_ = ("bank_admin", "appointment_catogry_lookup")
    ...
    appointments=Set('AppoiontmentInfo')
    PrimaryKey(id,key)
...
with db_session:
  ...
  select(p.key for p in AppointmentCatogry if  p.findByKeyName(value))

  # persons who have a car
  select(p.status for p in AppointmentCatogry if  p.status(True))