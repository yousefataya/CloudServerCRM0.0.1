...
class TransactionType(db.Entity):
    _table_ = ("bank_admin", "transaction_type_lookup")
    ...
    transactions=Set('BankTransaction')
    PrimaryKey(id)
    ...
    def status(self , status):
        select(p.key for p in TransactionType  if  self.status == status)

    @property
    @db_session(serializable=True)
    def findByKeyName(self , key):
        return select(p.key for p in TransactionType if self.key == key)
with db_session:
  # BankInfo who have a Id
  select(p.key for p in TransactionType if  p.findByKeyName(value))

  # persons who have a car
  select(p.status for p in TransactionType if  p.status(True))
